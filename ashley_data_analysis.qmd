...
growth_rates <- readxl::read_excel("output/table_S1_all_data.xlsx", sheet = "growth_rates")

# get the enzyme summary from S2
enzymes_sum <- readxl::read_excel("output/table_S2_data_summary.xlsx", sheet = "enzymes_summary")

# load the stats
stats <- readxl::read_excel("output/table_S3_stats.xlsx")
...
# or for all
enzymes_w_growth_rates |>
  mutate(enzyme = as_factor(enzyme)) |>
  ggplot() + 
  aes(growth_rate.1_d, rate_mean, color = dataset) +
  # york regression
  geom_line(
    data = function(df) 
      stats |> 
      mutate(
        dataset = str_extract(dataset, "^\\w+"),
        enzyme = str_remove(y, "_rate$")
      ) |>
      filter(
        dataset != "all",
        x == "growth_rate.1_d", 
        enzyme %in% unique(df$enzyme)) |>
      generate_york_regression_line(xmin = 0, xmax = 3),
    map = aes(x = yrl_x, y = yrl_y), linetype = 1
  ) +
  # summary data points with error bars
  geom_point(size = 4) +
  geom_errorbar(
  ...
  geom_errorbarh(
    aes(xmin = growth_rate.1_d - growth_rate_sd.1_d,
        xmax = growth_rate.1_d + growth_rate_sd.1_d),
    height = 0
  ) +
  scale_color_manual(values = c("glycerol" = blue, "glucose" = brown)) +
  theme_bw() +
