...
enzyme_calculations <- 
  ...
  mutate(
    ...
    cIDH_rate_sd = cIDH_rate * IDH_rate_sd/IDH_rate,
...
corr_data <- 
  all_data |> 
  mutate(`1/all_ab.ug_mg` = 1/all_ab.ug_mg) |>
  pivot_longer(
    c(
      all_eps.permil, growth_rate.1_d, `% cyto IDH`, `% total IDH`, `CN`, `% unsat`, `all_ab.ug_mg`, `1/all_ab.ug_mg`, `C16:1/C16:0_eps.permil`
    )
  ) |>
  select(dataset, growth, name, value) |>
  mutate(name = as_factor(name))

corr_matrix <- corr_data |>
  full_join(corr_data, by = c("dataset", "growth"), multiple = "all") |>
  ...
  group_by(select_dataset, name.x, name.y) |>
  summarize(
    n = n(),
    pearson_r = cor(value.x, value.y, method = "pearson"),
    pearson_pval = cor.test(value.x, value.y, method = "pearson")$p.value,
    lm_r2 = summary(lm(value.y ~ value.x))$r.squared,
    .groups = "drop"
  ) 
...
corr_table <- 
  corr_matrix |>
  select(dataset = select_dataset, y = name.y, x = name.x, n, pearson_r, pearson_pval, lm_r2)
...
$$
\begin{aligned}
f &= L \cdot \frac{m_{lipids}}{m_{cell}} = L \cdot ab \\
...
\rightarrow \epsilon_{lipids/water} + 1 &= \frac{\epsilon_{cell/water} + 1}{f \cdot (1 - \alpha_{o/l}) + \alpha_{o/l}} \\
\rightarrow \epsilon_{lipids/water} &= \frac{\epsilon_{cell/water} + 1}{ L \cdot ab \cdot (1 - \alpha_{o/l}) + \alpha_{o/l}} - 1
\end{aligned}
$$
...
## SI figure - correlation coefficient figure
...
```{r}
#| label: fig-SI-glucose-parameter-space
#| fig-width: 4.5
#| fig-height: 8
#| warning: false
fits |>
