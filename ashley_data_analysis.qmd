---
title: "Figure suggestions for Ashley's paper"
...
vars <- c(
  "all_eps.permil", "C16:1/C16:0_eps.permil", "1/(all_eps.permil + 1)", 
...
all_data_w_addons <- 
  ...
  mutate(
    ...
    `1/(all_eps.permil + 1)` = 1/(all_eps.permil/1000 + 1),
    `1/(all_eps.permil + 1) sd` = `1/(all_eps.permil + 1)` * all_eps_sd.permil/1000 * 1/(all_eps.permil/1000 + 1)
  )
...
export_to_excel(
  stats = stats_table,
  file = "output/table_S3_correlation_coefficients.xlsx"
)
...
# Data Overview (just to check, not for MS)

## Figure 2

```{r}
lipids |> 
  left_join(growth_rates, by = c("dataset", "growth")) |>
  ggplot() +
  aes(growth_rate.1_d, eps_mean.permil, color = dataset, shape = fatty_acid) +
  geom_point(
    data = function(df) df |> filter(fatty_acid == "all"),
    size = 4
  ) +
  # geom_point(
  #   data = function(df) df |> filter(fatty_acid != "all"),
  ...
  # ) +
  geom_jitter(
    data = lipids_all |> left_join(growth_rates, by = c("dataset", "growth")),
    map = aes(y = eps_fa_water.permil),
    size = 2, alpha = 0.5, show.legend = FALSE
  ) + 
  theme_bw() +
...
enzymes |> 
  left_join(growth_rates, by = c("dataset", "growth")) |>
  ggplot() +
  aes(growth_rate.1_d, rate_mean, color = dataset) +
  geom_smooth(method = "lm", formula = y ~ x, se = FALSE) +
  geom_point(size = 4) +
  geom_point(
    data = enzymes_all |>left_join(growth_rates, by = c("dataset", "growth")),
    map = aes(y = rate),
    size = 2, alpha = 0.5
  ) +
  theme_bw() +
...
## SI figure - york regressions figure

```{r}
#| label: fig-SI-eps-all-york-regressions
#| fig-width: 14
#| fig-height: 6
#| warning: false

# p value significance levels - use?
pval_levels <- 
...
stats_base_plot <-
  ...
  # panel highlight
  geom_rect(
    data = function(df) df |> filter(pearson_pval < 0.2, york_pval < 0.2),
  ...
  facet_grid(select_dataset + name.y ~ name.x, scales = "free", switch = "x") +
...
stats_base_plot %+%
  filter(
    ...
    !name.x %in% c("all_eps.permil", "1/(all_eps.permil + 1)", "C16:1/C16:0_eps.permil")
  ) +
  labs(y = latex2exp::TeX("$\\epsilon_{fattyacid/water}$ (\U2030)"))
```

```{r}
#| label: fig-SI-1-over-eps-york-regressions
#| fig-width: 14
#| fig-height: 6
#| warning: false
...
stats_base_plot %+%
  filter(
    stats,
    !is.na(chisq),
    name.y == "1/(all_eps.permil + 1)",
    !name.x %in% c("all_eps.permil", "1/(all_eps.permil + 1)", "C16:1/C16:0_eps.permil")
  ) +
  labs(y = latex2exp::TeX("$\\frac{1}{\\epsilon_{fattyacid/water} + 1}$"))
```
...
stats_base_plot %+%
  ...
  labs(y = latex2exp::TeX("$\\epsilon_{16{:}1/16{:}0}$ (\U2030)"))
```

## MAYBE SI figure - correlation coefficient figure

```{r}
#| label: fig-SI-york-reg-p-vals
#| fig-width: 8
#| fig-height: 6
#| warning: false
#| eval: false
...
corr_matrix |>
  filter(
    !is.na(chisq),
    name.y %in% c("all_eps.permil", "C16:1/C16:0_eps.permil"),
    !name.x %in% c("all_eps.permil", "C16:1/C16:0_eps.permil")
  ) |>
  mutate(
    name.y = fct_rev(name.y),
    signif = pearson_pval |> map_chr(~names(pval_levels)[which(.x < pval_levels)[1]]),
    p_val = stats::pchisq(chisq, chisq_df)
  ) |>
  ggplot() +
  aes(name.x, name.y) +
  #geom_tile(map = aes(fill = pearson_r)) +
  geom_tile(color = "black", fill = NA_character_) +
  #scale_fill_gradientn(colours = c("#6D9EC1", "white", "#E46726")) +
  #geom_point(map = aes(size = p_val, color = pearson_r > 0), alpha = 0.5) + 
  geom_point(map = aes(size = log10(pearson_pval), color = pearson_r > 0), alpha = 0.5) + 
  
  geom_text(map = aes(label = round(p_val, 2))) +
  scale_color_manual(values = c("#6D9EC1", "#E46726"), labels = function(x) c("neg. correlated", "pos. correlated")) +
...
corr_matrix |>
  ...
  #   data = function(df) df |> 
  #     filter(
  #       (name.x == "all_eps.permil" | name.y == "all_eps.permil") & pearson_pval < 0.01 |
  #       (
  #         name.x == "all_eps.permil" & name.y == "1/all_ab.ug_mg" & select_dataset == "glucose only"
  #       )
  #     ),
  #   color = NA_character_, fill = "yellow", alpha = 0.5
  # ) +
...
## Summary figure - eps vs t/cIDH

```{r}
#| label: fig-eps-vs-cidh
#| fig-width: 4
#| fig-height: 4
#| warning: false
p_vs_cidh <- 
  all_data |>
  ggplot() +
  aes(`% cyto IDH`, all_eps.permil) +
  geom_smooth(method = "lm", formula = y ~ x, se = FALSE, color = "black") + 
  geom_point(map = aes(color = dataset), size = 4) +
  # vertical error bars are smaller than sample points
  geom_errorbarh(
    map = aes(xmin = `% cyto IDH` - `% cyto IDH sd`,
              xmax = `% cyto IDH` + `% cyto IDH sd`,
              color = dataset),
    height = 0
  ) +
  ...
  scale_color_manual(values = c("glycerol" = blue, "glucose" = brown)) +
  labs(
    x = "estimated NADPH from cytosolic IDH", 
    y = latex2exp::TeX("$\\epsilon_{fattyacid/water}$ (\U2030)")
  )
p_vs_cidh 
```
...
p_glucose_fit <- 
  ...
  mutate(
    all_eps_model.permil = rlang::eval_tidy(eq)
  ) |>
  ggplot() + 
  aes(all_ab.ug_mg/1000, all_eps.permil, color = dataset) +
  geom_errorbar(
  ...
  geom_point() + 
  geom_line(map = aes(y = all_eps_model.permil), linewidth = 1) + 
  geom_point(size = 4) +
  scale_color_manual(values = c("glucose" = brown)) +
...
## Summary figure - combined

```{r}
#| label: fig-summary-cidh
#| fig-width: 9
#| fig-height: 4.5
#| warning: false
plot_grid(
...
## SI figure - NLS fit parameter space
...
p_eps_vs_lipid_ab_model <-
  ...
  mutate(
    name = name |> factor() |>
      fct_recode(
        "$\\epsilon_{other}$" = "eps_ow",
        "$\\epsilon_{cell}$" = "eps_cell",
        "$\\epsilon_{lipids}$" = "eps_lw"
      )
  ) |>
  ggplot() +
  aes(x = all_ab.ug_mg/1000, all_eps.permil) +
  # model & annotations
  geom_line(map = aes(linetype = name), color = brown, show.legend = FALSE) +
  geom_text(
    data = function(df) df |>
      summarize(
  ...
  labs(
    x = "weight % lipid\n(mg fatty acids / mg dry biomass)",
    y = latex2exp::TeX("$\\epsilon_{x/water}$ (\U2030)"),
    color = NULL
  ) +
  coord_cartesian(expand = FALSE)
p_eps_vs_lipid_ab_model
...
#| fig-height: 4.5
#| warning: false
p_eps_vs_lipid_ab_model_params <-
  fits |>
  mutate(panel = as_factor(term) |> fct_recode(
    "$\\epsilon_{cell/water}$ (\U2030)" = "eps_cell",
...
plot_grid(
  p_eps_vs_lipid_ab_model_params, 
  p_eps_vs_lipid_ab_model, 
  align = "h", nrow = 1, rel_widths = c(2, 1),
...
## SI figure - unsaturation fractionation

not entirely sure what to include here and how to make sense of it

```{r}
#| label: fig-SI-unsaturated-fractionation
#| fig-width: 10
#| fig-height: 4
#| warning: false
all_data |>
  mutate(
    # multiply for easier plotting
    `%unsat value` = `% unsat` * 100,
    `%unsat sd` = `% unsat sd` * 100,
    `growth_rate value` = growth_rate.1_d,
    `eps_all value` = all_eps.permil,
    `eps_all sd` = all_eps_sd.permil
  ) |>
  pivot_longer(
    cols = c(
      `growth_rate value`, `eps_all value`, `eps_all sd`, 
      `%unsat value`, `%unsat sd`),
    names_to = c("name", ".value"),
    names_pattern = "([^ .]+) (value|sd)?"
  ) |> 
  mutate(
    panel = name |> as_factor() |>
      fct_recode(
        "Growth rate ($d^{-1}$)" = "growth_rate",
        #"abundance ratio\n(16:1/16:0)" = "r_16_1_0"
        "Unsaturated FAs (%)" = "%unsat"
      )
    ) |>
  ggplot() +
  aes(value, `C16:1/C16:0_eps.permil`, color = dataset) +
  geom_errorbar(
    map = aes(ymin = `C16:1/C16:0_eps.permil` - `C16:1/C16:0_eps_sd.permil`,
              ymax = `C16:1/C16:0_eps.permil` + `C16:1/C16:0_eps_sd.permil`),
    width = 0
  ) +
  geom_errorbarh(
    map = aes(xmin = value - sd, xmax = value + sd),
    height = 0
  ) +
  geom_smooth(method = "lm", formula = y ~ x, se = FALSE) + 
  geom_point(size = 4) +
  ...
  labs(x = NULL, y = latex2exp::TeX("$\\epsilon_{16{:}1/16{:}0}$ (\U2030)")) 
```
