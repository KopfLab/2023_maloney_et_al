...
generate_york_regression_line <- function(ds, expand = ggplot2::expansion(mult = 0.1), n = 10L) {
  ...
  ds |>
    dplyr::mutate(
      # xmin
      yrl_xmin1 = expand_range(xmin, xmax, expand[1], expand[2], -1),
      yrl_xmin2 = expand_range(calc_x_from_y(ymin, a, b), calc_x_from_y(ymax, a, b), expand[1], expand[2], -1),
      yrl_xmin = ifelse(yrl_xmin1 > yrl_xmin2, yrl_xmin1, yrl_xmin2),
      # xmax
      yrl_xmax1 = expand_range(xmin, xmax, expand[3], expand[4], +1),
      yrl_xmax2 = expand_range(calc_x_from_y(ymin, a, b), calc_x_from_y(ymax, a, b), expand[3], expand[4], +1),
      yrl_xmax = ifelse(yrl_xmax1 < yrl_xmax2, yrl_xmax1, yrl_xmax2)
    ) |>
